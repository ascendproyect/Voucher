...
public abstract class Menu {
    ...
    private ItemStack createItemStack (Player player, Button button){
        ItemStack item = button.getItemStack(player);
    ...
    public void openMenu (Player player,boolean allowPlayerInventoryClick){
        this.buttons = this.getMenuContent(player);
        Menu previousMenu = Menu.currentlyOpenedMenus.get(player.getName());
        Inventory inventory = null;
        int size = (this.getSize() == -1) ? this.size(this.buttons) : this.getSize();
        boolean update = false;
        String title = CC.translate(this.getName(player));
        if (title.length() > 32) {
        ...
        if (this.isPlaceholder()) {
            for (int index = 0; index < size; ++index) {
                if (this.buttons.get(index) == null) {
                    ...
                    inventory.setItem(index, this.placeholderButton.getItemStack(player));
                }
            }
        }
        if (update) {
    ...
    public Map<Integer, Button> getMenuContent () {
        return this.buttons;
    }
